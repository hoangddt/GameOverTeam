...
{
	mVertexShader = loadShader(GL_VERTEX_SHADER, vertexShaderPath);
	if ( mVertexShader == 0 )
	{
		printf("\nLoading %s fail", vertexShaderPath);
		return -1;
	}
	mFragmentShader = loadShader(GL_FRAGMENT_SHADER, fragmentShaderPath);
	...
	this->mAttrNormal     = glGetUniformLocation ( this->mProgram, "a_normal" );
