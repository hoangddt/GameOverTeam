...
void main()
{
	...
	vec3 u_lightPos = vec3(0.0, -2.0, -2.0);
	...
	vec4 specular = pow(max(dot(reflectVector, toEye), 0.0), 8.0) * lightSpecColor;

	//gl_FragColor = u_transparent * mix(diffuse, texture1, 0.5) + (1.0 - u_transparent) * diffuse + ambient;
	gl_FragColor = diffuse * 1.2 + ambient;
}